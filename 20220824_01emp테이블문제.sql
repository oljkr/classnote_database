...
select * from emp;
--자바같이 외부 프로그램과 연동할 때는 *(와일드카드)를 쓰면 안 됨.
--db에서 직접볼 때는 칼럼명을 볼 수 있는데 자바같은 곳에서 조회하면
--칼럼명이 안 보임
...
alter table emp add 연봉 int;
select * from emp;
alter table emp drop (연봉);
update emp set 연봉 = sal*12 + comm;
update emp set 연봉 = sal*12 where comm is null;

select ename 이름, deptno 부서, sal 급여, comm 커미션,(sal*12+comm) as 연봉
from emp;
...
--comm칼럼에 null 값을 0으로 바꾸기
select ename, nvl(comm,0)
from emp
where comm is null;

--comm 칼럼에 null 값을 0으로 바꾼후, 연봉 계산하기
select ename, deptno, sal, sal*12+nvl(comm,0) as 연봉
from emp;

--소수점 없이 반올림해서 연봉 계산하기
...
문28) 사번, 이름, 부서를 조회하시오
      (단, 부서코드가 10이면 관리부
                      20이면 영업부
                      30이면 교육부로 바꿔서 출력);
select empno 사번, ename 이름, case deptno
                              when 10 then '관리부'
                              when 20 then '영업부'
                              when 30 then '교육부'
                              end as 부서
...
문29) 다음의 SQL문을 분석하시오
      select empno, sal 
      from emp
      where not(sal>200 and sal<300) --not(201~299)급여가 201~299이 아닌 행 조회
      order by sal;

급여가 200보다 크지 않거나(조건1) 300보다 같거나 큰(조건2) 직원의 사번, 급여를 급여 순으로 조회
조건1과 조건2 중 하나만 충족시키면 됨.
...
급여가 200보다 크지 않고(조건1) 300보다 작은(조건2) 직원의 사번, 급여를 급여 순으로 조회
조건1과 조건2를 모두 충족시켜야 함.
여기서 not은 sal>200에만 적용됨.
...
--변환함수 : to_char(원래 날짜, '원하는 모양')

--입사일 조회
select hiredate from emp; --입사일 조회

--입사일에서 월 가져오기
select to_char(hiredate, 'mm') from emp;
select to_char(hiredate, 'mm') as 월 from emp;
select to_char(hiredate, 'mm') as 월, count(*) from emp
...
select to_char(hiredate, 'mm'), count(*) as 인원수
from emp
group by to_char(hiredate, 'mm')
order by count(*);
...
문41) 아래와 같이 출력 하시오
      예) 박지성의 근속년수 : 20년
select ename ||'의 근속년수 : '|| (extract(year from sysdate)-extract(year from hiredate))||'년'
from emp;
...
select empno, ename, hiredate, (sal*12+nvl(comm,0)) 현재연봉, (sal*12+nvl(comm,0))*1.1 인상후연봉, (sal*12+nvl(comm,0))*0.1 인상된금액
from emp
